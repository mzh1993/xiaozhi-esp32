...
#### 4. 板载应用执行流程(boards/bread-compact-wifi)

**板载应用架构分析：**

**1. CompactWifiBoard 类 (boards/bread-compact-wifi/compact_wifi_board.cc):**

- **继承关系**: `CompactWifiBoard` 继承自 `WifiBoard`，这是一个特定于WiFi功能的板卡实现。
- **显示初始化**: 使用I2C总线初始化SSD1306 OLED显示屏，配置I2C参数并创建显示面板。
- **按钮初始化**: 初始化多个按钮，包括启动按钮、触摸按钮、音量增减按钮，并为每个按钮设置点击和长按事件的回调函数。
- **物联网初始化**: 使用 `ThingManager` 添加可见的物联网设备，如扬声器和灯。
- **音频编解码器**: 提供 `GetAudioCodec()` 方法，返回音频编解码器实例，支持简单和复杂的I2S音频方法。
- **LED控制**: 提供 `GetLed()` 方法，返回单个LED实例，用于设备状态指示。

**2. Board 类 (boards/common/board.cc 和 board.h):**

- **单例模式**: `Board` 类使用单例模式，通过 `GetInstance()` 方法获取唯一实例。
- **UUID生成**: 使用ESP32的硬件随机数生成器创建设备唯一标识符(UUID)，并存储在设置中。
- **设备信息获取**: 提供 `GetJson()` 方法，返回设备的详细信息，包括UUID、芯片信息、应用程序信息和分区表。
- **虚函数接口**: 定义多个纯虚函数，如 `GetBoardType()`、`GetAudioCodec()`、`CreateHttp()` 等，供具体板卡实现。
- **电源管理**: 提供 `SetPowerSaveMode()` 方法，用于控制设备的省电模式。

**架构总结：**

- `CompactWifiBoard` 类实现了具体的板卡功能，包括显示、按钮、音频和物联网设备的初始化和控制。
- `Board` 类提供了通用的板卡接口和设备信息管理，支持不同板卡的具体实现。
- 这种架构设计使得不同的板卡可以通过继承和实现 `Board` 类的接口来适配不同的硬件特性，具有良好的扩展性和可维护性。
