...
static const char TAG[] = "ToyAudioCodecLite";

ToyAudioCodecLite::ToyAudioCodecLite(void* i2c_master_handle, i2c_port_t i2c_port, int input_sample_rate, int output_sample_rate,
    gpio_num_t mclk, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din,
    gpio_num_t pa_pin, uint8_t es8311_addr, bool use_mclk, bool input_reference) {
    duplex_ = true; // 是否双工
    ...
    // ESP_LOGI("input_channels_ = ", input_channels_);
...
ToyAudioCodecLite::~ToyAudioCodecLite() {
...
void ToyAudioCodecLite::CreateDuplexChannels(gpio_num_t mclk, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din) {
...
void ToyAudioCodecLite::SetOutputVolume(int volume) {
    ESP_ERROR_CHECK(esp_codec_dev_set_out_vol(output_dev_, volume));
    AudioCodec::SetOutputVolume(volume);
}

void ToyAudioCodecLite::EnableInput(bool enable) {
...
void ToyAudioCodecLite::EnableOutput(bool enable) {
...
int ToyAudioCodecLite::Read(int16_t* dest, int samples) {
    if (input_enabled_) {
        if (!input_reference_) {
...
int ToyAudioCodecLite::Write(const int16_t* data, int samples) {
    if (output_enabled_) {
